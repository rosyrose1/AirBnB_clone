...
class TestState_instance(unittest.TestCase):
    ...
    def test_str_representation(self):
        ...
        ststr = st.__str__()
        self.assertIn("[State] (ssty54)", ststr)
        self.assertIn("'id': 'ssty54'", ststr)
    ...
    def test_instantiation_with_kwargs(self):
        dt = datetime.today()
        dt_iso = dt.isoformat()
        st = State(id="007", created_at=dt_iso, updated_at=dt_iso)
        self.assertEqual(st.id, "007")
...
class TestState_to_dict(unittest.TestCase):
    ...
    def test_to_dict_contains_added_attributes(self):
        st = State()
        st.middle_name = "Arkansas"
        st.my_number = 29
        self.assertEqual("Arkansas", st.middle_name)
